...
void	init_player(t_player *player, t_data *data)
{
	...
	player->rotate_angle = check_angle(data->player->player_derection);
...
int	main(int ac, char **av)
{
	...
	if (ac == 2)
	{
		...
		ft_data_init(&data);
		mlx_hook(data.win_ptr, 02, 0, key_press, &data);
		mlx_hook(data.win_ptr, 17, 0, ft_destroy_window, &data);
