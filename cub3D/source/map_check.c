...
static void	ft_update_plyr(t_var *var, int nbr_plyrs, int x, int y)
{
	t_map	*obj_map;
...
void	is_characters(t_var *var, t_map *obj_map)
{
	int		x;
	int		y;
	int		nbr_plyrs;
	...
	while (map[++y])
	{
		...
		while (map[y][++x])
		{
			...
			{
				nbr_plyrs++;
				ft_update_plyr(var, nbr_plyrs, x, y);
			}
		}
	}
	if (nbr_plyrs == 0)
...
void	wall_checker(t_var *var, t_map *obj_map)
{
	int		x;
	int		y;
	char	**map;
